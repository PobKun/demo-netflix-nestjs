pipeline {
    ...
    stages {
        ...
        stage('Build') {
            environment {
                IMAGE_NAME = "demo_netflix_nestjs"
            }
            steps {
        ...
        stage('Deploy') {
             environment {
                ...
                SSH_SCRIPT = """
                    cd project && \
                    sudo IMAGE_TAG=${PAYLOAD_TAGNAME} docker compose -f docker-compose.demo_netflix.yaml -f docker-compose.main.yaml up -d demo_netflix_nestjs nginx
                """
            }
            steps {
